...
func createProject(project models.SocialMedia) (*models.SocialMedia, error) {
	tx := database.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
...
func getProject(filter SocialRequest, pageSize string, pageNumber string) ([]models.SocialMedia, int64, int, int, error) {
	// Set default values for page size and page number
	perPage := 15
	page := 1
	...
	// Initialize database model with filtering conditions
	db := database.Model(&models.SocialMedia{})
...
func getSingleProject(ID uuid.UUID) (*models.SocialMedia, error) {
	var record models.SocialMedia
	if err := database.
		First(&record, "id = ?", ID).Error; err != nil {
		return nil, err
	}
...
func updateProject(ID uuid.UUID, user models.User, name string) (*models.SocialMedia, error) {
	tx := database.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
	...
	var existingRecord models.SocialMedia
	if err := tx.First(&existingRecord, "id = ?", ID).Error; err != nil {
		return nil, err // Record not found or other database error
	}
...
func deleteSingleProject(ID uuid.UUID) error {
	...
	result := tx.Delete(&models.SocialMedia{}, "id = ?", ID)
	if result.Error != nil {
		// Rollback the transaction if an error occurs
		tx.Rollback()
		return result.Error
